...
function findLastEvenNumber(arr) {
	...
	if (Array.isArray(arr) === false) {
		...
		let lastEvenNumber = arr.findLast(function (element) {
			return element % 2 === 0;
		});
...
function getArrayKeys(arr) {
	...
	let arrayKeys = Array.from(arr.keys());
	return arrayKeys;
}
...
function removeAndDouble(arr, startIdx) {
	...
	if (Array.isArray(arr) === false) {
		...
		let doubledArray = arr.splice(startIdx).map((value) => value * 2);
		return doubledArray;
	}
}

console.log("Завдання: 7 ==============================");
console.log(removeAndDouble([1, 2, 3, 4, 5], 3)); // Виведе [ 2, 4 ]
...
function getLastElementsSumAndPrint(arr, n) {
	...
	if (Array.isArray(arr) === false) {
		...
		arr.slice(-n).forEach((value) => console.log(value));
		return sum;
	}
}
...
function wordLengths(arr) {
	...
	if (Array.isArray(arr) === false) {
		...
		let lengthsArray = arr.map((value) => value.length);
		return lengthsArray;
	}
}
...
function expandArrayByN(arr, n) {
	...
	if (Array.isArray(arr) === false || typeof n !== "number") {
...
function findLongestWord(arr) {
	...
	if (Array.isArray(arr) === false) {
		...
		let longestWord = arr.reduce((previousWord, currentWord) => {
...
function findLastNegativeNumberIndex(arr) {
	...
	if (Array.isArray(arr) === false) {
		...
		let lastIndex = arr.findLastIndex((item) => {
			return item < 0;
		});
		return lastIndex;
	}
